  ...
  switch (request.type) {
    ...
    case 'APPROVE_USER':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          const uid = result.user?.uid;
          const orgId = result.activeOrgId;
          const canApprove = await roleManager.can(uid, orgId, 'members.approve');
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user'], async (result) => {
          ...
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false }); return; }
            await orgManager.requestToJoin(user.uid, request.orgId, user.displayName, user.email, token);
            sendResponse({ ok: true });
          } catch (e) {
            sendResponse({ ok: false, error: e.message });
          }
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId', 'members', 'roles'], async (result) => {
          ...
          const canInvite = await permissionManager.hasPermission(uid, orgId, 'invite_users') ||
                            await permissionManager.hasPermission(uid, orgId, 'members.invite');
    ...
    case 'GET_ORG_MEMBERS':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          const uid = result.user?.uid;
          // Accept orgId from request for hierarchy rendering, fall back to activeOrgId
          const orgId = request.orgId || result.activeOrgId;
          if (!uid || !orgId) { sendResponse({ ok: false, data: [] }); return; }
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, data: [] }); return; }
    ...
    case 'GET_ORG_ROLES':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          const orgId = request.orgId || result.activeOrgId;
          if (!orgId) { sendResponse({ ok: false, data: [] }); return; }
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, data: [] }); return; }
            const roles = await roleManager.getRolesForOrg(orgId, token);
    ...
    case 'UPDATE_ROLE_V2':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          const uid = result.user?.uid;
          const orgId = result.activeOrgId;
          if (!uid || !orgId) { sendResponse({ ok: false, error: 'Missing data' }); return; }
          const canEdit = await roleManager.can(uid, orgId, 'roles.edit');
          if (!canEdit) { sendResponse({ ok: false, error: 'Unauthorized' }); return; }
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            await roleManager.updateRoleStructure(request.roleId, {
              name: request.name,
              level: request.level,
              color: request.color,
    ...
    case 'CAN_DO':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          const uid = result.user?.uid;
          const orgId = result.activeOrgId;
          if (!uid || !orgId) { sendResponse({ ok: true, can: false }); return; }
          try {
            const can = await roleManager.can(uid, orgId, request.permission);
            sendResponse({ ok: true, can });
          } catch (e) {
            sendResponse({ ok: false, can: false, error: e.message });
          }
        });
      });
      return true;
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            await orgManager.leaveOrganization(uid, orgId, token);
            // If we left the active org, clear it
            if (result.activeOrgId === orgId) {
              await new Promise(r => chrome.storage.local.remove(['activeOrgId'], r));
            }
            await storageManager.fullSync();
            sendResponse({ ok: true });
          } catch (e) {
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            await orgManager.deleteOrganization(uid, orgId, token);
            if (result.activeOrgId === orgId) {
              await new Promise(r => chrome.storage.local.remove(['activeOrgId'], r));
            }
            await storageManager.fullSync();
    ...
    case 'CHANGE_MEMBER_ROLE':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          const orgId = result.activeOrgId;
          if (!uid || !orgId) { sendResponse({ ok: false, error: 'Missing data' }); return; }
          const canChange = await roleManager.can(uid, orgId, 'members.changeRoles');
          if (!canChange) { sendResponse({ ok: false, error: 'Unauthorized' }); return; }
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            await orgManager.changeMemberRole(orgId, request.targetUserId, request.newRoleId, uid, token);
            await storageManager.fullSync();
            sendResponse({ ok: true });
          } catch (e) {
            sendResponse({ ok: false, error: e.message });
    ...
    case 'KICK_MEMBER':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          const orgId = result.activeOrgId;
          if (!uid || !orgId) { sendResponse({ ok: false, error: 'Missing data' }); return; }
          const canKick = await roleManager.can(uid, orgId, 'members.kick');
          if (!canKick) { sendResponse({ ok: false, error: 'Unauthorized' }); return; }
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            await orgManager.kickMember(orgId, request.targetUserId, uid, token);
            await storageManager.fullSync();
            sendResponse({ ok: true });
          } catch (e) {
            sendResponse({ ok: false, error: e.message });
    ...
    case 'CREATE_ROLE_V2':
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          const orgId = result.activeOrgId;
          if (!uid || !orgId) { sendResponse({ ok: false, error: 'Missing data' }); return; }
          const canCreate = await roleManager.can(uid, orgId, 'roles.create');
          if (!canCreate) { sendResponse({ ok: false, error: 'Unauthorized' }); return; }
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            const role = await roleManager.createRole(
              orgId,
              request.name,
              request.permissions, // can be nested object or array
              request.level || 50,
              request.color || '#00e5ff',
              token
            );
            await storageManager.fullSync();
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, error: 'Token expired' }); return; }
            await orgManager.approveJoinRequest(orgId, request.requestId, token);
            await storageManager.fullSync();
            sendResponse({ ok: true });
          } catch (e) {
            sendResponse({ ok: false, error: e.message });
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          try {
            const token = await storageManager.getValidToken();
            if (!token) { sendResponse({ ok: false, data: [] }); return; }
            const requests = await firestoreRest.queryDocuments('joinRequests',
              { field: 'orgId', op: 'EQUAL', value: orgId }, token).catch(() => []);
            sendResponse({ ok: true, data: requests.filter(r => r.status === 'pending') });
          } catch (e) {
            sendResponse({ ok: false, error: e.message, data: [] });
          }
        });
      });
      return true;
      ...
      ensureAccount(() => {
        chrome.storage.local.get(['user', 'activeOrgId'], async (result) => {
          ...
          try {
            ...
            await firestoreRest.deleteDocument(`joinRequests/${request.requestId}`, token);
            sendResponse({ ok: true });
          } catch (e) {
            sendResponse({ ok: false, error: e.message });
          }
