...
class StorageManager {
  ...
  async _pullSessions(uid, members, roles, token) {
    ...
    // Supports both legacy flat arrays and new nested objects
    function hasPerm(permissions, permName) {
      if (!permissions) return false;
      if (Array.isArray(permissions)) {
        return permissions.includes(permName);
      }
      if (typeof permissions === 'object') {
        // Check legacy keys against nested structure
        // e.g. 'view_all_sessions' -> sessions.read (approximate mapping)
        if (permName === 'view_all_sessions') return permissions.sessions?.read === true;
        if (permName === 'view_team_sessions') return permissions.sessions?.read === true;
        if (permName === 'view_shared_sessions') return permissions.sessions?.read === true;
        if (permName === 'view_own_sessions') return permissions.sessions?.read === true;
        // Dot-notation lookup
