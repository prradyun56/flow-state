document.addEventListener('DOMContentLoaded', () => {
  ...
  if (createRoleBtn) {
    createRoleBtn.addEventListener('click', () => {
      ...
      document.getElementById('create-role-color').value = '#00e5ff';
      buildPermGrid();
  ...
  function openEditRoleModal(role) {
    ...
    document.getElementById('create-role-color').value = role.color || '#00e5ff';
  ...
  if (submitRoleBtn) {
    submitRoleBtn.addEventListener('click', () => {
      ...
      const color = document.getElementById('create-role-color').value || '#00e5ff';
      ...
      chrome.runtime.sendMessage({ type: 'CAN_DO', permission: 'roles.edit' }, (editResp) => {
        ...
        sorted.forEach(role => {
          ...
          if (Array.isArray(perms)) {
            // Legacy array - map to readable labels
            const LEGACY_LABELS = {
          ...
          const editBtn = canEdit
            ? `<button class="dash-btn role-edit-btn" data-role-id="${role._id}" style="font-size:10px; padding:4px 12px; margin-left:auto;">✎ Edit</button>`
            : '';

          card.innerHTML = `
            <div class="role-card-header">
              <span class="role-card-name" style="color:${rc}; text-shadow:0 0 5px ${rc}50">${esc(role.name)}</span>
              <span style="font-size:10px; color:#666; margin-left:8px;">L${role.level ?? 0}</span>
