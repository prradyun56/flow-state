document.addEventListener('DOMContentLoaded', () => {
  ...
  const flaggedDomainsListEl = document.getElementById('flagged-domains-list');
  ...
  saveBtn.addEventListener('click', () => {
    ...
    if (visibility !== 'private') {
      chrome.tabs.query({ currentWindow: true }, (tabs) => {
        ...
        if (hasSensitive) {
          if (confirm('Warning: Some tabs appear to contain login pages or sensitive information.\n\nAre you sure you want to share this session?')) {
            proceedSave();
          }
        } else {
          proceedSave();
  ...
  function renderPrivacyPanel() {
    ...
    // flaggedTabs is now [{id, url, title, hostname}]
    ...
    flaggedDomainsListEl.innerHTML = '';
    const hasFlags = privacyState.flaggedTabs.length > 0 || privacyState.flaggedDomains.length > 0;

    if (!hasFlags) {
      flaggedDomainsListEl.innerHTML = '<div class="privacy-empty">NO_FLAGS_ACTIVE</div>';
      return;
    }

    // --- Flagged tabs section ---
    if (privacyState.flaggedTabs.length > 0) {
      const tabHeader = document.createElement('div');
      tabHeader.className = 'privacy-list-header';
      tabHeader.textContent = 'FLAGGED TABS';
      flaggedDomainsListEl.appendChild(tabHeader);

      privacyState.flaggedTabs.forEach(tabInfo => {
        const display = tabInfo.hostname || tabInfo.title || `tab #${tabInfo.id}`;
        const row = document.createElement('div');
        row.className = 'flagged-domain-row';
        row.innerHTML = `
          <div class="flagged-entry-info">
            <span class="flagged-domain-name">${escapeHtml(display)}</span>
            <span class="flagged-entry-sub">${escapeHtml(tabInfo.title || tabInfo.url || '(closed tab)')}</span>
          </div>
          <button class="icon-btn remove-domain-btn" data-tabid="${tabInfo.id}" title="Unflag this tab">✕</button>
        `;
        flaggedDomainsListEl.appendChild(row);
      });
    }

    // --- Flagged domains section ---
    if (privacyState.flaggedDomains.length > 0) {
      const domHeader = document.createElement('div');
      domHeader.className = 'privacy-list-header';
      domHeader.textContent = 'FLAGGED DOMAINS';
      flaggedDomainsListEl.appendChild(domHeader);

      privacyState.flaggedDomains.forEach(domain => {
        const row = document.createElement('div');
        row.className = 'flagged-domain-row';
        row.innerHTML = `
          <div class="flagged-entry-info">
            <span class="flagged-domain-name">${escapeHtml(domain)}</span>
            <span class="flagged-entry-sub">all subdomains included</span>
          </div>
          <button class="icon-btn remove-domain-btn" data-domain="${escapeHtml(domain)}" title="Unflag this domain">✕</button>
        `;
        flaggedDomainsListEl.appendChild(row);
      });
    }
    ...
    // Bind remove buttons
    flaggedDomainsListEl.querySelectorAll('.remove-domain-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        if (btn.dataset.tabid) {
          const tabId = parseInt(btn.dataset.tabid, 10);
          chrome.runtime.sendMessage({ type: 'TOGGLE_TAB_FLAG', tabId }, (resp) => {
            if (resp && resp.ok) {
              loadPrivacyState();
              showToast('Tab unflagged.', 'success');
            }
          });
        } else if (btn.dataset.domain) {
          chrome.runtime.sendMessage({ type: 'REMOVE_DOMAIN_FLAG', domain: btn.dataset.domain }, (resp) => {
            if (resp && resp.ok) {
              loadPrivacyState();
              showToast(`"${btn.dataset.domain}" unflagged.`, 'success');
            }
          });
        }
      });
  ...
  // Toggle privacy panel visibility
  ...
  addDomainBtn.addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ type: 'ADD_DOMAIN_FLAG', domain }, (resp) => {
      if (resp && resp.ok) {
        domainInput.value = '';
        loadPrivacyState();
        showToast(`"${domain}" flagged.`, 'success');
      }
    });
  });
  ...
  // Load privacy state summary on startup (keeps badge up-to-date even with panel closed)
  loadPrivacyState();
    ...
    ['board', 'sc', 'jc'].forEach(rank => {
      if (groups[rank].length > 0) {
        ...
        groups[rank].forEach(session => {
          ...
          // Restore button — own sessions always, others only if visibility is 'edit' (write)
